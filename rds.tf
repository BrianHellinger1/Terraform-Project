...
resource "aws_db_instance" "brians_database" {
  allocated_storage      = 10
  engine                 = "mysql"
  engine_version         = "5.7"
  instance_class         = "db.t2.micro"
  db_subnet_group_name   = aws_db_subnet_group.brians-week22-subgroup.id
  vpc_security_group_ids = [aws_security_group.Week22-Public-SG-DB.id]
  name                   = "brians_database"
...
resource "aws_security_group" "brians_database_tier_lu" {
  name        = "brians_database_tier_lu"
